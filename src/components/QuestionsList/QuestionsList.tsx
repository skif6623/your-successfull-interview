...
export const QuestionsList: FC<IQuestionListProps> = ({title}) => {
	...
	const getVisibleQuestions = () => {
		...
		return sortedQuestions.categoryItems.filter(item => item.question.toLowerCase().includes(normalizeFilter));
	};
				...
				{visibleQuestions.map(({id, question, answer}: IQItem) => {
					return (
						<EQuestItem key={id} onClick={() => toggleAnswer(id)}>
							<EQuestTitle color={sortedQuestions.color}>{question}</EQuestTitle>
